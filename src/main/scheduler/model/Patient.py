...
class Patient:
    ...
    # Insert appointment that patient made
    def upload_appointment(self, d, id, cusername,vname):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        ...
        try:
            cursor.execute(add_appointment, (id, cusername, d, vname ,self.username))
            # you must call commit() to persist your data if you don't set autocommit to True
            conn.commit()
        except pymssql.Error:
            print("Error occurred when updating appointment")
            cm.close_connection()
        cm.close_connection()
    
    def delete_ava(self, d, username):
        cm = ConnectionManager()
        conn = cm.create_connection()
        cursor = conn.cursor()
        ...
        try:
            cursor.execute(delete, (username,d))
            # you must call commit() to persist your data if you don't set autocommit to True
            conn.commit()
        except pymssql.Error:
            print("Error occurred when deleting availabilities")
            cm.close_connection()
        cm.close_connection()
